...
def exportTreeNode(nodeCtx: NodeContext):
    ...
    if node.BitmapBuffer.use_external_image:
        ...
        if node.texture and (image := node.texture.image):
            filePath = image_utils.getTrackedImagePath(image)

            if image_utils.imageUpdated(image):
                # Force update the file attribute to force reload of the image file
                # Note: During GPU rendering, this workaround is ineffective.
                #       Removing and re-exporting the plugin or exporting the "file" attribute with forceUpdate
                #       also does not refresh the texture in GPU mode.
                #       The texture won't be updated in that mode until a work around is found.
